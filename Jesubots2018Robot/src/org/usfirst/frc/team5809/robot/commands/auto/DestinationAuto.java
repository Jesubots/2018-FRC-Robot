...
public class DestinationAuto extends CommandGroup {
	...
	public DestinationAuto() {
		...
		if (OI.getDestination().getFieldDistance() == eFieldDistance.kUnknown) {
			...
			addSequential(new DriveStraightDistance(Destination.NearSideDestination.DRIVE_SEG1.getDriveData(), 3.0));
			addSequential(new PivotTurn(0.45, Destination.NearSideDestination.DRIVE_TURN1.getDriveData(), 2.0, false));
			addSequential(new DriveStraightDistance(Destination.NearSideDestination.DRIVE_SEG2.getDriveData(), 2.0));
			...
			addSequential(new DriveStraightTime(1.5, 0.65));
